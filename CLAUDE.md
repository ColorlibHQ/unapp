...
| Sections, page starters, hidden template partials | `patterns/*.php` (74 files) |
...
- **Starter sites** (`inc/starter-sites.php`) are the demo system: each definition names a curated style variation, a home pattern, supporting pages, swatches and a header CTA label. `unapp_apply_starter_site()` creates pages, writes the variation into the user's `wp_global_styles` post, rebuilds the `wp_navigation` post and saves a customised `header` template part when the CTA differs from the theme default (and deletes that part when it matches again, so switching back reverts cleanly). It never deletes user content. Extend via the `unapp_starter_sites` filter.
