...
<div class="feature-section action-required demo-import-boxed" id="plugin-filter">
	...
	if ( ! empty( $this->actions ) ) {
		...
		foreach ( $this->actions as $key => $value ) {
			...
			<div class="action-required-box">
				<span data-action="<?php print $hidden ? 'visible' : 'hidden'; ?>" class="dashicons <?php print $hidden ? 'dashicons-hidden' : 'dashicons-visibility'; ?> required-action-button" id="<?php echo esc_attr( $value['id'] ); ?>">
				</span>
	...
	<?php if ( $hooray ) { ?>
		<span class="hooray"><?php echo esc_html__( 'Hooray! There are no required actions for you right now.', 'unapp' ); ?> </span>
	<?php } ?>
