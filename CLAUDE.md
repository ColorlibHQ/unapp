...
- Avatars: `.dev/avatars.py` generates `assets/images/avatars/*.svg` — duotone silhouettes, one muted ground and one figure tone each, with a different hair and shoulder shape per file so a row of four reads as four people. Long hair is drawn *behind* the head and shoulders; anything drawn after the head is clipped to the skull, or it leaves a slab across the chin.
