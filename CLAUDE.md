...
- **No orphaned wraps.** A flex/grid row must never leave a single item alone on the last line. WP's grid layout keeps one column count at every width, so use the `unapp-grid-4` / `unapp-grid-3` utility classes (in `style.css`) alongside `columnCount`, which step down through counts that divide the item count evenly. `.dev`-adjacent tooling: the wrap audit script groups children by vertical *overlap* (not exact `top`, which false-positives on rows of mixed-height items) and flags `orphan` (last row = 1) and `ragged` (last row < full row).
