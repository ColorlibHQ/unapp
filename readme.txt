...
= 2.5.7 - 2026-09-17 =
* Fixed: the Finance starter's copy contradicted itself — the home page says the firm has been independent since 1998 while the adviser's biography said she founded it in 2009. She now joined in 2009 and has led it since.
...
* Fixed: five niche call-to-action patterns used the category "call to action" where WordPress registers "call-to-action", so they were missing from that group in the inserter.

= 2.5.2 - 2026-08-20 =
* The companion plugin now sends the same update payload as the theme, including the one-way site identifier, so plugin installs can be counted too. It honours the same unapp_check_for_updates opt-out.
* The update disclosure on Appearance → Starter Sites now covers the plugin when it is active.

= 2.5.1 - 2026-08-20 =
* Fixed: the site tagline is displayed again. Every footer now renders it under the site title when one is set, and keeps its written line as a fallback when it is not.
* Fixed: the theme description said six starter sites and 70+ patterns; there are thirteen and 140+.
