...
class Epsilon_Welcome_Screen {
		...
		if ( isset( $_POST['args'], $_POST['args']['nonce'] ) && ! wp_verify_nonce( sanitize_key( $_POST['args']['nonce'] ), 'welcome_nonce' ) ) {
			wp_die(
				wp_json_encode(
					array(
						'status' => false,
						'error'  => esc_html__( 'Not allowed', 'unapp' ),
					)
				)
			);
		}
		...
		if ( ! class_exists( $args_action[0] ) ) {
			wp_die(
				wp_json_encode(
					array(
						'status' => false,
						'error'  => esc_html__( 'Class does not exist', 'unapp' ),
					)
				)
			);
		}
		...
		if ( is_admin() ) {
			...
			wp_localize_script(
				...
				array(
					...
					'no_required_actions_text' => esc_html__( 'Hooray! There are no required actions for you right now.', 'unapp' ),
					'ajax_nonce'               => wp_create_nonce( 'welcome_nonce' ),
					'activating_string'        => esc_html__( 'Activating', 'unapp' ),
					'body_class'               => 'appearance_page_' . $this->theme_slug . '-welcome',
					'no_actions'               => esc_html__( 'Hooray! There are no required actions for you right now.', 'unapp' ),
				)
			);
		}
	}
	...
	private function add_admin_notice() {
		...
		if ( empty( $this->notice ) ) {
			...
			$this->notice .= '<h1>' . sprintf( esc_html__( 'Welcome to %1$s', 'unapp' ), $this->theme_name ) . '</h1>';
			$this->notice .= '<p>';
			$this->notice .=
			...
			$this->notice .= '<p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-welcome&tab=recommended-actions' ) ) . '" class="button button-primary button-hero" style="text-decoration: none;"> ' . sprintf( esc_html__( 'Get started with %1$s', 'unapp' ), $this->theme_name ) . '</a></p>';
	...
	public function welcome_screen_menu() {
		...
		$title = sprintf( esc_html__( 'About %1$s', 'unapp' ), esc_html( $this->theme_name ) );
		...
		<div class="wrap about-wrap epsilon-wrap">
			...
			<h2 class="nav-tab-wrapper wp-clearfix">
				<?php foreach ( $this->sections as $id => $section ) { ?>
					<a class="nav-tab <?php print $id === $tab ? 'nav-tab-active' : ''; ?>" href="<?php echo esc_url( $section['url'] ); ?>"><?php echo wp_kses_post( $section['label'] ); ?></a>
				<?php } ?>
			</h2>
	...
	private function set_default_sections( $config = array() ) {
		$arr = array(
			'getting-started'     => array(
				...
				'label' => esc_html__( 'Getting Started', 'unapp' ),
			...
			'recommended-actions' => array(
				...
				'label' => esc_html__( 'Recommended Actions', 'unapp' ),
			...
			'recommended-plugins' => array(
				...
				'label' => esc_html__( 'Recommended Plugins', 'unapp' ),
	...
	private function check_plugin( $slug = '' ) {
		$arr = array(
			...
			'label'     => esc_html__( 'Install and Activate', 'unapp' ),
		);
		...
		if ( $arr['installed'] ) {
			...
			$arr['label'] = esc_html__( 'Activate now', 'unapp' );
		}

		if ( $arr['active'] ) {
			$arr['needs'] = 'deactivate';
			$arr['class'] = 'deactivate-now button button-primary';
			$arr['label'] = esc_html__( 'Deactivate now', 'unapp' );
		}
	...
	public static function set_frontpage_to_static( $args = array() ) {
		...
		if ( null === $home ) {
			$id = wp_insert_post(
				array(
					'post_title'  => esc_html__( 'Homepage', 'unapp' ),
					'post_type'   => 'page',
					'post_status' => 'publish',
				)
			);
		...
		if ( null === $blog ) {
			$id = wp_insert_post(
				array(
					'post_title'  => esc_html__( 'Blog', 'unapp' ),
					'post_type'   => 'page',
					'post_status' => 'publish',
				)
			);
